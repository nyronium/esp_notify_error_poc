...
inline std::string ToString(const ble_uuid16_t& uuid) {
	char buf[32];
	snprintf(buf, sizeof(buf), "%04x", uuid.value);
	return buf;
}

inline std::string ToString(const ble_uuid32_t& uuid) {
	char buf[32];
	snprintf(buf, sizeof(buf), "%08x", uuid.value);
	return buf;
}
...
public:
	...
	inline static std::string ToString() {
		char buf[64];
		snprintf(buf, sizeof(buf), "%08x-%04x-%04x-%04x-%012x", A, B, C, D, E);
		return buf;
	}
};
