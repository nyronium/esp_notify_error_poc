...
inline static std::string AddrToString(uint8_t val[6]) {
	char buf[32];
	snprintf(buf, sizeof(buf), "%02x:%02x:%02x:%02x:%02x:%02x", val[0], val[1], val[2], val[3], val[4], val[5]);
	return buf;
}

inline static void BlePrintConnDesc(ble_gap_conn_desc& desc) {
	ESP_LOGI("BleInstance", "handle=%d our_ota_addr_type=%d our_ota_addr=%s"
			, desc.conn_handle, desc.our_ota_addr.type
			, AddrToString(desc.our_ota_addr.val).c_str());
	ESP_LOGI("BleInstance", "our_id_addr_type=%d our_id_addr=%s"
			, desc.our_id_addr.type
			, AddrToString(desc.our_id_addr.val).c_str());
	ESP_LOGI("BleInstance", "peer_ota_addr_type=%d peer_ota_addr=%s"
			, desc.peer_ota_addr.type
			, AddrToString(desc.peer_ota_addr.val).c_str());
	ESP_LOGI("BleInstance", "peer_id_addr_type=%d peer_id_addr=%s"
			, desc.peer_id_addr.type
			, AddrToString(desc.peer_id_addr.val).c_str());
	ESP_LOGI("BleInstance", "conn_itvl=%d conn_latency=%d supervision_timeout=%d encrypted=%d authenticated=%d bonded=%d"
			, desc.conn_itvl
			, desc.conn_latency
...
inline static void NimbleHostTask(void* param) {
	ESP_LOGI("BleInstance", "NimbleHostTask started");
	...
	~Instance() noexcept {
		...
		if (auto err = nimble_port_stop(); err == 0) {
			...
			if (auto err = esp_nimble_hci_and_controller_deinit(); err != ESP_OK) {
				ESP_LOGE("BleInstance", "error in esp_nimble_hci_and_controller_deinit(): %s", esp_err_to_name(err));
			}
		}
	}
...
private:
	...
	void OnReset(int reason) {
		ESP_LOGE("BleInstance", "Resetting state; reason=%d", reason);
		inSync = false;
	}
	...
	void OnSync() {
		...
		if (auto rc = ble_hs_id_infer_auto(0, &ownAddressType); rc != 0) {
			ESP_LOGE("BleInstance", "error determining address type; rc=%d", rc);
			return;
		}
		...
		ESP_LOGI("BleInstance", "Device Address: %s", AddrToString(addrVal.data()).c_str());
	...
	int OnGapEvent(ble_gap_event* event) {
		switch (event->type) {
			case BLE_GAP_EVENT_CONNECT:
				...
				ESP_LOGI("BleInstance", "connection %s; status=%d ",
			...
			case BLE_GAP_EVENT_DISCONNECT:
				ESP_LOGI("BleInstance", "disconnect; reason=%d ", event->disconnect.reason);
			...
			case BLE_GAP_EVENT_CONN_UPDATE: {
				...
				ESP_LOGI("BleInstance", "connection updated; status=%d ", event->conn_update.status);
				ble_gap_conn_desc desc;
				esp::check(ble_gap_conn_find(event->connect.conn_handle, &desc), "ble_gap_conn_find()");
				BlePrintConnDesc(desc);
				return 0;
			...
			case BLE_GAP_EVENT_ADV_COMPLETE:
				ESP_LOGI("BleInstance", "advertise complete; reason=%d", event->adv_complete.reason);
				EnableAdvertising();
				return 0;

			case BLE_GAP_EVENT_ENC_CHANGE: {
				// Encryption has been enabled or disabled for this connection.
				ESP_LOGI("BleInstance", "encryption change event; status=%d ", event->enc_change.status);
				ble_gap_conn_desc desc;
				esp::check(ble_gap_conn_find(event->connect.conn_handle, &desc), "ble_gap_conn_find()");
				BlePrintConnDesc(desc);
				return 0;
			...
			case BLE_GAP_EVENT_SUBSCRIBE:
				ESP_LOGI("BleInstance", "subscribe event; conn_handle=%d attr_handle=%d "
			...
			case BLE_GAP_EVENT_MTU:
				ESP_LOGI("BleInstance", "mtu update event; conn_handle=%d cid=%d mtu=%d",
			...
			case BLE_GAP_EVENT_PASSKEY_ACTION: {
				ESP_LOGI("BleInstance", "PASSKEY_ACTION_EVENT started");
				ble_sm_io pkey{};
				//int key = 0;
				...
				if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
					...
					ESP_LOGI("BleInstance", "Enter passkey %d on the peer side", pkey.passkey);
					int rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
					ESP_LOGI("BleInstance", "ble_sm_inject_io result: %d", rc);
				} else if (event->passkey.params.action == BLE_SM_IOACT_NUMCMP) {
					ESP_LOGI("BleInstance", "Passkey on device's display: %d", event->passkey.params.numcmp);
					ESP_LOGI("BleInstance", "Accept or reject the passkey through console in this format -> key Y or key N");
					pkey.action = event->passkey.params.action;
					//if (scli_receive_key(&key)) {
					//	pkey.numcmp_accept = key;
					//} else {
					//	pkey.numcmp_accept = 0;
					//	ESP_LOGE("BleInstance", "Timeout! Rejecting the key");
					//}
					int rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
					ESP_LOGI("BleInstance", "ble_sm_inject_io result: %d", rc);
					...
					ESP_LOGI("BleInstance", "Enter the passkey through console in this format-> key 123456");
					pkey.action = event->passkey.params.action;
					//if (scli_receive_key(&key)) {
					//	pkey.passkey = key;
					//} else {
					//	pkey.passkey = 0;
					//	ESP_LOGE("BleInstance", "Timeout! Passing 0 as the key");
					//}
					int rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
					ESP_LOGI("BleInstance", "ble_sm_inject_io result: %d", rc);
				}
	...
	static void StaticOnRegister(ble_gatt_register_ctxt* context, void* arg) {
		switch (context->op) {
			case BLE_GATT_REGISTER_OP_SVC:
				ESP_LOGI("BleInstance", "Registered service: uuid=%s, handle=%d",
						ToString(context->svc.svc_def->uuid).c_str(), context->svc.handle);
				break;

			case BLE_GATT_REGISTER_OP_CHR:
				ESP_LOGI("BleInstance", "Registered characteristic: uuid=%s, def_handle=%d, val_handle=%d",
						ToString(context->chr.chr_def->uuid).c_str(),
						context->chr.def_handle,
						context->chr.val_handle);
				break;

			case BLE_GATT_REGISTER_OP_DSC:
				ESP_LOGI("BleInstance", "Registered descriptor: uuid=%s, handle=%d",
						ToString(context->dsc.dsc_def->uuid).c_str(), context->dsc.handle);
				break;
